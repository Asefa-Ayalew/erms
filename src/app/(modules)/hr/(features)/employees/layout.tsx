...
import { useLazyGetEmployeeQuery, useLazyGetEmployeesQuery } from "./_store/employee.query";
...
export default function EmployeesLayoutPage({ children }: { children: React.ReactNode }) {
  ...
  const [getEmployees, { data: employees, isLoading: isLoadingEmployees }] = useLazyGetEmployeesQuery();
  ...
  const config = useMemo<EntityConfig<Employee>>(
    () => ({
      primaryColumn: {
        key: "firstName",
        name: "Name",
        render: (row: Employee) => `${row.firstName ?? ""} ${row.employeeCode ? `· ${row.employeeCode}` : ""}`.trim(),
      },
      ...
      visibleColumn: [
        ...
        },
        {
          ...
          name: "Status",
          render: (row: Employee) => row.employmentStatus ?? "",
        },
      ],
      hasActions: false,
      showDetail: true,
  ...
  const detailLabel =
    params.id === "new"
      ? "New employee"
      : [employee?.firstName, employee?.employeeCode].filter(Boolean).join(" · ") || "Employee";
